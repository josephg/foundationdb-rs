...
/// interface uses `usize` if it represents length or size of something. Those `usize` values should
/// be converted to `c_int` before being passed to ffi, because naive casting with `v as i32` will
...
impl Transaction {
    ...
    pub fn get_key<'a>(&self, selector: KeySelector, snapshot: bool) -> impl WaitFuture<Result<FutCell<'a, &'a [u8]>>> {
        ...
        unsafe {
            let f = fdb::fdb_transaction_get_key(
                trx,
                key.as_ptr() as *const _,
                key.len() as i32,
...
//    }
    ...
    pub fn get_addresses_for_key<'a>(&self, key: &[u8]) -> impl WaitFuture<Result<FutCell<'a, Vec<&'a [u8]>>>> {
        ...
        unsafe {
            let f = fdb::fdb_transaction_get_addresses_for_key(
                trx,
                key.as_ptr() as *const _,
                key.len() as i32,
            );
